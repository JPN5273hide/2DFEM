...
int approximation(int cnn[72][3], double node_coord_x[49], double node_coord_y[49], double u[49]){
    ...
    */
    ...
    for (int i=0; i<m; i++){
        resid[i] = tmpf[i];
        for (int j=0; j<m; j++)  resid[i] -= tmpA[i][j]*x[j];
    }
    double dir[m];
    for (int i=0; i<m; i++) dir[i] = resid[i];
    double resid_norm = 0;
    for (int i=0; i<m; i++) resid_norm += resid[i]*resid[i];
    resid_norm = sqrt(resid_norm);
    while (resid_norm >= 1e-5){
        for (int i=0; i<m; i++) u[non_boarder_node[i]] = x[i];
        printf("\n%lf \n", resid_norm);
        for (int i=0; i<m; i++) printf("%lf ", x[i]);
        printf("\n");
        for (int i=0; i<m; i++) printf("%lf ", resid[i]);
        double alpha, num_alpha=0, den_alpha=0;
        for (int i=0; i<m; i++){
            num_alpha += resid[i]*resid[i];
            double den_alpha_tmp = 0;
            for (int j=0; j<m; j++) den_alpha_tmp += tmpA[i][j]*dir[j];
            den_alpha += den_alpha_tmp*dir[i];
        }
        alpha = num_alpha/den_alpha;
        double prev_resid[m];
        for (int i=0; i<m; i++) prev_resid[i] = resid[i];
        for (int i=0; i<m; i++){
            x[i] += alpha*dir[i];
            for (int j=0; j<m; j++){
                resid[i] -= alpha*tmpA[i][j]*dir[j];
            }
        }
        double beta, num_beta=0, den_beta=0;
        for (int i=0; i<m; i++){
            num_beta += resid[i]*resid[i];
            den_beta += prev_resid[i]*prev_resid[i];
        }
